...
void BaseControlNode::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    ControlStatus innerStatus;

    if (std::abs(actual_linear_vel_) > epsilon || std::abs(actual_angular_vel_) > epsilon) {
        innerStatus = ControlStatus::RUNNING;
        last_cmd_time_ = this->now(); 
    } else if ((this->now() - last_cmd_time_).seconds() > 3.0) {
        innerStatus = ControlStatus::IDLE;
    } else if (std::abs(actual_linear_vel_) < epsilon && std::abs(actual_angular_vel_) < epsilon) {
        innerStatus = ControlStatus::STOPPED;
    } else {
        innerStatus = ControlStatus::READY_OK;
    }
